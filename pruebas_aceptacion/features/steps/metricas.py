...
def step_impl(context):
    ...
    denied_keywords = [
        ...
        '403']
    assert any(
        k in body for k in denied_keywords), f"No redirigido a login ni encontrado mensaje de denegación. URL: {current} Texto: {body[:200]}"
...
def step_impl(context):
    ...
    try:
        table = context.driver.find_element(
            By.XPATH,
            "//h3[normalize-space()='Solicitudes por tipo']/following-sibling::table[1]")
...
def step_total_exact(context, expected):
    ...
    assert str(actual) == str(
        expected), f"Total de tickets esperado {expected} pero se encontró {actual}"
...
def step_en_proceso(context, expected):
    ...
    if m:
        actual = m.group(1)
        assert str(actual) == str(
            expected), f"En proceso esperado {expected} pero se encontró {actual}"
        return
    # Si no se encuentra, falla con pista para preparar la plantilla
    raise AssertionError(
        "No se encontró el elemento 'En proceso' en la página. Asegúrate de que la plantilla muestra 'En proceso: <n>' o ajusta el selector")
...
def step_responsable_count(context, name, count):
    ...
    try:
        table = context.driver.find_element(
            By.XPATH,
            "//h3[normalize-space()='Solicitudes por responsable']/following-sibling::table[1]")
    except Exception:
        ...
        for line in body:
            if name in line:
                ...
                if m:
                    actual = m.group(1)
                    assert str(actual) == str(
                        count), f"Para responsable {name} se esperaba {count} pero se encontró {actual} en la línea: {line}"
                    return
        raise AssertionError(
    ...
    for tr in rows[1:]:
        ...
        if first == name:
            ...
            assert str(actual) == str(
                count), f"Para responsable {name} se esperaba {count} pero se encontró {actual}"
            return
    raise AssertionError(
