...
class FormularioSolicitud(models.Model):
    tipo_solicitud = models.OneToOneField(
        TipoSolicitud, on_delete=models.CASCADE, related_name='formulario')
    nombre = models.CharField(max_length=150)
...
class CampoFormulario(models.Model):
    formulario = models.ForeignKey(
        FormularioSolicitud, on_delete=models.CASCADE, related_name='campos')
...
class SeguimientoSolicitud(models.Model):
    solicitud = models.ForeignKey(
        Solicitud, on_delete=models.CASCADE, related_name='seguimientos')
    fecha_creacion = models.DateTimeField(auto_now_add=True)
